...
#
# Vendored into pt-nuts from blackjax/progress_bar.py: this module has no
# dependency on any BlackJAX sampler API (it only instruments jax.lax.scan
# via jaxtap), so it works unmodified now that pt-nuts uses NumPyro's own
# HMC/NUTS kernel instead of BlackJAX's.
"""A ``jaxtap``-powered progress bar for jax.lax.scan-based loops.
...
them into a :class:`ProgressState` display loop.

Import cost: importing this module does **not** import ``jaxtap``. The
import happens lazily inside :func:`progress_bar`'s ``__enter__`` so a
plain install without the optional `jax-tap` package continues to work.
"""
import os
import threading
import time
    ...
    def _step_callback(self, idx):
        ...
        try:
                ...
                if self.output_file:
                    try:
                        ...
                        if not already_warned:
                            try:
                                warnings.warn(
                                    "pt_nuts.progress_bar: disabling "
            ...
            if not already_warned:
                try:
                    warnings.warn(
                        "pt_nuts.progress_bar: internal callback error "
...
@contextmanager
def progress_bar(label="pt-nuts", print_rate=None, output_file=None):
    """Add a progress bar to any jax.lax.scan-based sampling loop.
    ...
    algorithm being run.  Emission is powered by the optional ``jax-tap``
    package (imported as ``jaxtap``).
    ...
    output_file
        ...
        ``python -m pt_nuts.progress_reader <path>`` (if you vendor that
        reader too; the file format is just ``"<step> <total>"``).
    ...
    * A user's own ``jaxtap.record()`` context opened *inside* this context
    ...
    .. code::
        ...
        with progress_bar(label="NUTS warmup"):
            state = jax.lax.scan(step_fn, init_state, None, length=1000)
    """
    try:
        import jaxtap as tap
    except ImportError as exc:
        raise ImportError(
            "pt_nuts.progress_bar requires the optional 'jax-tap' package. "
            "Install it with:  pip install jax-tap"
        ) from exc
    ...
    try:
        ...
        if state.n_steps == 0:
            warnings.warn(
                "pt_nuts.progress_bar: no scan step was ever observed "
